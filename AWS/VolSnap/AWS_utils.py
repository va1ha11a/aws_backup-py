...
class SnapUtils:
    _ec2_conn = boto.ec2.connect_to_region(
    ...
    def _bt_to_dt(self, boto_time):
        ...
        logger.debug("Convert time from boto format to python datetime: " +
                     str(boto_time))
    ...
    def _get_bu_tags_from_snap(self, snapshot):
        ...
        tags = {
            bu_tag: distutils.util.strtobool(snapshot.tags.get(val, "False"))
            for bu_tag, val in settings.snap_tags.iteritems()
        }
        return tags
    ...
    def get_vols_for_backup(self):
        ...
        all_vols = self._ec2_conn.get_all_volumes(
            filters={"tag:" + settings.backup_enabled_tag: True})
        return [{
            "id": vol.id,
            "policy": vol.tags[settings.backup_policy_tag]
        } for vol in all_vols]
    ...
    def get_snaps_for_vol(self, volume_id):
        ...
        logger.info("Getting list of existing snapshots for volume: " +
                    str(volume_id))
        snaps = self._ec2_conn.get_all_snapshots(
            filters={
                "volume_id": volume_id,
        ...
        snaps_details = [{
            "bu_keys": self._get_bu_tags_from_snap(snap),
    ...
    def delete_snapshot(self, snap_id, dry_run=False):
        snapshot = self._ec2_conn.get_all_snapshots(
            filters={'snapshot_id': snap_id})[0]
        if not dry_run:
    ...
    def create_snap_for_vol(self, volume_id, bu_keys):
        ...
        logger.info("Create a snapshot and tag as per due. Volume: " +
                    str(volume_id))
...
class MailUtils:
    ...
    _ses_conn = boto.ses.connect_to_region(
        settings.ses_region,
        aws_access_key_id=settings.aws_access_key_id,
        aws_secret_access_key=settings.aws_secret_access_key)
