...
def expires_tomorrow():
    ...
    doc_heading = "AWS Backups due to expire between: %s - %s (UTC)" % (
        str(now_utc), str(tomorrow_utc))
...
def vols_not_snapped_today():
    ...
    details = [{
        'vol_id': vol,
        'last': date
    } for vol, date in latest.iteritems()
               if date == None or date < yesterday_utc]
    cols = ['vol_id', 'last']
    headers = ['Volume ID', 'Last Snapshot Time']
    doc_heading = "AWS Volumes with no successfull snapshots since: %s (UTC)" % (
        str(yesterday_utc))
    result = generic_template.render(details=details,
                                     cols=cols,
                                     headers=headers,
...
def mail_report(title, func, mailer):
    ...
    html = func()
    results = mailer.send_mail(settings.email_from,
                               title,
                               html,
                               settings.email_to,
                               html_body=html)
...
report_map = {
    "Expiry Report": expires_tomorrow,
